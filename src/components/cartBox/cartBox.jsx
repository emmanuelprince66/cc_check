        ...
        sx={{
          ...
          border:
            currentTheme.palette.type === "light"
              ...
              : "1px solid #2C2C2E",
          backgroundColor:
            currentTheme.palette.type === "light" ? "#FAFAFA" : "#2C2C2E",
          borderRadius: "0 0 .3em .3em ",
        }}
      >
        <div style={{ fontSize: "12px", whiteSpace: "nowrap" }}>
          <span style={{ fontWeight: "700" }}> N {itemInfo.subTotal} </span>
          <span style={{ color: "#727272", fontSize: "10px" }}>
            {" "}
            /{itemInfo.count} PORTION
          </span>
        </div>
        <Box
          display="flex"
        ...
        >
          {(itemInfo?.added &&
            orders[id - 1]?.menu[index].id === itemInfo.id) ||
          itemInfo.canEditPreview ? (
            <Box
              display="flex"
              sx={{ width: "100%" }}
              gap={".3em"}
            ...
            <Box sx={{ width: "100%", gap: ".2em", display: "flex" }}>
              <Box
                sx={{
                  display: "flex",
                  justifyContent: "space-between",
          ...
          ) : itemInfo.canPreview ? null : null}
        </Box>
      </Box>
      {showRemoveModal ? (
        <RemoveOrderModal
          onRemoveClick={handleRemoveItemFromCart}
